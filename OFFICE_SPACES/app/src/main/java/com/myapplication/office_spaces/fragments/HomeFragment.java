...
public class HomeFragment extends Fragment {
    ...
    private void setupClicks(View view) {
        ...
        ) -> { if (actionId == EditorInfo.IME_ACTION_SEARCH) { submitHomeSearch(); return true; } return false; });
    ...
    private void submitHomeSearch() {
        String query = etHomeSearch.getText() != null ? etHomeSearch.getText().toString().trim() : "";
        if (query.isEmpty()) {
        ...
        intent.putExtra(SearchFragment.EXTRA_LISTING_TYPE, "All");
        intent.putExtra(SearchFragment.EXTRA_MAX_PRICE, Float.MAX_VALUE);
        intent.putExtra(SearchFragment.EXTRA_MAX_AREA, Float.MAX_VALUE);
        intent.putExtra(SearchFragment.EXTRA_RADIUS_KM, -1f); startActivity(intent);
