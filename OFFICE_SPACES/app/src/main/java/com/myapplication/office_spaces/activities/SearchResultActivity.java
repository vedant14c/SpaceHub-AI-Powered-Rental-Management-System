...
public class SearchResultActivity extends AppCompatActivity {
    ...
    private void loadAndFilterProperties() {
        ...
        if (query != null && !query.trim().isEmpty()) {
            // Free-text query typed by the user -> let the AI (Gemini) endpoint
            // interpret it into filters (city, type, price, etc.) server-side.
            runSmartSearch();
        } else {
            // No free-text query -> use the manual dropdown/slider filters
            // the user picked on the Search screen, same as before.
    ...
    private void runSmartSearch() {
        ApiClient.getApiService(this)
                .smartSearch(new SmartSearchRequest(query.trim()))
                .enqueue(new Callback<List<Property>>() {
                    ...
                    public void onResponse(Call<List<Property>> call, Response<List<Property>> response) {
                        ...
                        if (!response.isSuccessful() || response.body() == null) {
                            // AI search failed (e.g. Gemini key/quota issue) -> fall back
                            // to the plain manual search instead of leaving a blank screen.
                            runManualFilterSearch();
                            return;
                        }

                        filteredProperties.clear();
                        filteredProperties.addAll(response.body());
