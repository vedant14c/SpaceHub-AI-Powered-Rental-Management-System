...
public class AddListingActivity extends AppCompatActivity {
    ...
    /** Coordinates picked via GPS or the map picker take priority over forward-geocoding the typed address. */
    ...
    private final ActivityResultLauncher<Intent> mapPickerLauncher =
            registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
                ...
                pickedLatitude = data.getDoubleExtra(LocationPickerActivity.EXTRA_LATITUDE, 0);
                pickedLongitude = data.getDoubleExtra(LocationPickerActivity.EXTRA_LONGITUDE, 0);

                String address = data.getStringExtra(LocationPickerActivity.EXTRA_ADDRESS);
                String city = data.getStringExtra(LocationPickerActivity.EXTRA_CITY);
                String state = data.getStringExtra(LocationPickerActivity.EXTRA_STATE);

                if (address != null) etAddress.setText(address);
                if (city != null) etCity.setText(city);
                if (state != null) etState.setText(state);

                showLocationStatus(String.format(Locale.getDefault(),
                        "Location pinned on map (%.5f, %.5f)", pickedLatitude, pickedLongitude));
            });
    ...
    private void setupClicks() {
        ...
        findViewById(R.id.btnUseCurrentLocation).setOnClickListener(v -> useCurrentLocation());
        findViewById(R.id.btnPickOnMap).setOnClickListener(v ->
                mapPickerLauncher.launch(new Intent(this, LocationPickerActivity.class)));
    ...
    /** Fetches the device's current GPS location and reverse-geocodes it to fill Address/City/State. */
    private void useCurrentLocation() {
        ...
        locationHelper.requestPermissionAndFetch(new LocationHelper.LocationCallback() {
            ...
            public void onLocationResult(android.location.Location location, String city, String fullAddress) {
                pickedLatitude = location.getLatitude();
                pickedLongitude = location.getLongitude();

                if (fullAddress != null) etAddress.setText(fullAddress);
                if (city != null) etCity.setText(city);

                showLocationStatus(String.format(Locale.getDefault(),
                        "Using current location (%.5f, %.5f)", pickedLatitude, pickedLongitude));
            }

            @Override
            public void onLocationUnavailable(String reason) {
                showLocationStatus(null);
                Toast.makeText(AddListingActivity.this, reason, Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    private void showLocationStatus(String message) {
        if (message == null) {
            txtLocationStatus.setVisibility(View.GONE);
            return;
        }
        txtLocationStatus.setText(message);
        txtLocationStatus.setVisibility(View.VISIBLE);
    }
    ...
    private void submitListing() {
        ...
        // Prefer coordinates the owner explicitly picked (GPS / map picker) since they're
        // more accurate than forward-geocoding a hand-typed address. Fall back to
        // geocoding the address only if nothing was picked.
        if (pickedLatitude != null && pickedLongitude != null) {
            property.setLatitude(pickedLatitude);
            property.setLongitude(pickedLongitude);
        } else {
            try {
