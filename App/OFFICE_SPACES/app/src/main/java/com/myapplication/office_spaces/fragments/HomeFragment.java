...
public class HomeFragment extends Fragment {
    ...
    private void fetchUserLocation() {
        try {
            // Safety timeout: If GPS doesn't respond in 8 seconds, show error
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                if (isAdded() && userLocation == null && "Detecting location...".equals(txtUserLocation.getText().toString())) {
                    txtUserLocation.setText("Location unavailable");
                }
            }, 8000);
    ...
    private float calculateDistance(Property p) {
        if (userLocation == null || p.getLatitude() == null || p.getLongitude() == null 
    ...
    private void setupClicks(View view) {
        view.findViewById(R.id.searchBar).setOnClickListener(v ->
                ((RenterMainActivity) requireActivity()).navigateToSearch("All"));
